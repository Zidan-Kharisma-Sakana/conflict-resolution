<form class="" action="{{ route('musyawarah.update', $musyawarah->id) }}" method="post"
    ...
    @if ($user->role == \App\Models\User::IS_PIALANG && empty($musyawarah->hasil))
        <textarea required name="hasil" class="mt-1 w-full border-gray-300 rounded-md shadow-sm max-w-4xl">{{ old('hasil', $musyawarah->hasil) }}</textarea>
        @error('hasil')
    ...
    @else
        <textarea name="hasil" disabled class="mt-1 w-full border-gray-300 rounded-md shadow-sm max-w-4xl">{{ $musyawarah->hasil }}</textarea>
    @endif
</form>
